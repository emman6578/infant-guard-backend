...
const requiredFieldsCategory: string[] = ["name"];

// Function to check required fields and extra fields in the request body
export function checkRequiredFieldsCategory(req: any) {
  // Check if all required fields are present in req.body
  const missingFields = validateProductFields(req.body, requiredFieldsCategory);
  ...
  const extraFields = Object.keys(req.body).filter(
    (field) => !requiredFieldsCategory.includes(field)
  );
...
// Function to check required fields and extra fields in the request body
export function checkRequiredUpdateFieldsCategory(req: any) {
