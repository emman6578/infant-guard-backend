...
CREATE TABLE `Order` (
    `id` VARCHAR(191) NOT NULL,
    `total` DOUBLE NULL,
    `wholesale_price_total` DOUBLE NULL,
    ...
    UNIQUE INDEX `Order_admin_id_key`(`admin_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `DriverLoad` (
    ...
    `status` ENUM('FORSALE', 'SOLD', 'CONFIRMED') NOT NULL DEFAULT 'FORSALE',
...
CREATE TABLE `DriverSales` (
    ...
    `status` ENUM('FORSALE', 'SOLD', 'CONFIRMED') NOT NULL DEFAULT 'CONFIRMED',
